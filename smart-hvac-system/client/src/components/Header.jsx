...
export function Header({
  ...
  const { espConnected, rssi } = systemState;

  // Signal strength indicator
  const getRssiBars = (dbm) => {
    if (!espConnected) return 0;
    if (dbm >= -60) return 4;
    if (dbm >= -70) return 3;
    if (dbm >= -80) return 2;
    return 1;
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span className="badge-glass badge-blue" style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                v1.0 ESP32
              </span>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
          ...
          {espConnected ? (
            <div className="badge-glass badge-green" title={`ESP32 Hardware Online • RSSI: ${rssi || '--'} dBm`}>
              ...
              <div style={{ display: 'flex', gap: '2px', marginLeft: '6px', alignItems: 'flex-end', height: '12px' }}>
                {[1, 2, 3, 4].map(bar => (
                  <span
                    key={bar}
                    style={{
